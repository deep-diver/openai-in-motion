...
data/
  ...
  object-callouts.json          44개 장면의 오브젝트 콜아웃 220개
  cast-dialogue.json            장면별 인물 연출 대사 66개
  sceneNotes.ts                콜아웃과 대사의 타입이 지정된 진입점
...
components/timeline/
  ...
  StageAnnotations.tsx         3D 위치를 따라가는 연결선·콜아웃·인물 말풍선
  annotationLayout.ts          화면 크기에 맞춘 무대 가장자리 설명 배치
  StoryNarration.tsx           연속 탐색 막대·겹쳐 이어지는 자막
  storyClock.ts                모든 연출이 공유하는 시점과 대사 구간
  continuousMotion.ts          입력부터 결과까지 이어지는 빛·발화 동작
...
- 서사의 세 지점은 `beat-0`, `beat-1`, `beat-2`로 보존하되 하나의 시계로 연속 재생
- 세 번째 단계에서 이미 등장한 물체를 축소하지 않음
- 동작과 자막을 겹쳐 연결하고, 완성된 결과를 유지한 채 14.2초에 다음 장면으로 전환하거나 정지
...
각 장면에서 가장 중요한 오브젝트 **두 개만** 골라 이름과 짧은 설명을 붙입니다. 선택은 `SECONDARY_OBJECT`에서 사건별로 지정합니다. 인물이 말하는 동안 보조 설명은 잠시 사라져 말풍선까지 합쳐 최대 두 패널만 보입니다. 연결선 끝은 `localToWorld → camera.project`로 매 프레임 계산하여 움직이는 오브젝트를 따라갑니다. 화면 가장자리의 텍스트는 고정되어 읽기 편하게 유지되며, 나타나지 않은 오브젝트의 설명은 숨깁니다.

등장인물의 말풍선에도 연결선이 있습니다. 대사는 실제 인용이 아닌 **연출 대사**임을 말풍선에 명시했습니다. 인물의 입과 손짓은 말풍선과 같은 시계로 움직입니다. 인물 이름을 누르면 해당 대사 시점에서 멈추며, 동작 줄이기 모드에서도 모든 인물의 대사를 선택할 수 있습니다.
...
- 재생 / 일시정지, 현재 장면 다시 보기
- 1× / 1.5× / 2× 재생 속도
- 재생 막대 드래그로 현재 장면의 어느 시점이든 탐색
- 막대 아래의 작은 지점을 누르면 자막 전환이 완료된 시점까지 부드럽게 이동
- 인물 이름을 눌러 연출 대사 보기
...
콜아웃 220개와 인물 대사 66개의 완전성, 화면 크기별 설명 영역 배치, 연결선 끝점 변경, 자막의 연속성, 서사 경계에서 빛의 연속 이동, 슬라이더의 키보드 입력도 검사합니다.
